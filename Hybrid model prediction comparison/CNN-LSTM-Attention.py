...
# rcParams['font.sans-serif'] = ['SimHei']  # Use SimHei (黑体) font
# rcParams['axes.unicode_minus'] = False  # Display minus sign properly
# 读取数据
...
# 转换为Tensor
vp_train_tensor = torch.tensor(vp_train, dtype=torch.float32).reshape(-1,6,1)  # 增加时间步维度
...
class CNNLSTMAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        att_weights = torch.softmax(self.attention(x), dim=1)  # 注意力权重
        x = x * att_weights  # 加权
        x = x.mean(dim=1)  # 汇总
        x = self.fc(x)  # 全连接层
        return x
...
# -------------------- 保存 --------------------
save_path = r"D:\Desktop\1-1\预测图.jpg"
...
#
# # 保存为 Excel 文件
# output_file = r"D:\Desktop\CNN_LSTM_Attention预测结果.xlsx"
