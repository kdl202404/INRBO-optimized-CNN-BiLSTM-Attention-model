...
class CNNGRUAttention(nn.Module):
    def __init__(self):
        ...
        # self.gru = nn.GRU(input_size=64, hidden_size=128, num_layers=2,dropout=0.1, batch_first=True)
    ...
    def forward(self, x):
        ...
        x = x.view(x.size(0), -1, 64)  # 需要重塑
        x, _ = self.gru(x)  # 获取所有时间步的输出
        x = self.fc(x[:, -1, :])  # 取最后一步
        return x
...
# 保存为 Excel 文件
output_file = r"D:\Desktop\CNN_GRU预测结果.xlsx"
